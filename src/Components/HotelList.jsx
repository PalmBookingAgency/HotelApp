...
// Array of all the hotels, where each hotel is an object with various properties.
export const hotels = [{ 
...
// Function whichs finds the hotel by matching with the hotel ID.
export function getHotel(hotelID) {
  return hotels.find((hotel) => hotel.hotelID === parseInt(hotelID));
}

// Component that displays a hotel list. With the help of the "map" function, each hotel is traversed and they are rendered in a container with the various properties such as name, price, number of stars, etc.
// In each hotel box, a button is also rendered using the reusable button component.
// Through the Link component from React Router, the button is linked so that it leads to the specific page for a specific hotel.
